...
export default function IngredientsPage() {
  ...
  // ── GSAP: Clip-reveal the page heading once on mount ─────────────────────────
  useEffect(() => {
    return clipRevealOnScroll(headingRef.current, { start: "top 90%" });
  }, []);

  // ── GSAP: Staggered card reveal — re-runs when filter changes ────────────────
  useEffect(() => {
    if (!gridRef.current) return;
    const cards = Array.from(gridRef.current.children) as HTMLElement[];
    if (!cards.length) return;

    const cleanup = staggerRevealGrid(cards, { stagger: 0.06, yFrom: 32 });
    // Filter swap shifts layout — let ScrollTrigger recalc positions.
      ...
      <main className="flex-grow pt-28 pb-20 bg-primary-bg min-h-screen relative z-10 text-muted-text">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
            ...
            <h1
              ref={headingRef}
              className="text-4xl md:text-6xl font-bold tracking-tight text-cream-white"
            >
